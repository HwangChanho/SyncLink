 ...
 *
 * 2026-09-13 완화(LEAD 결정) — 실행 5→3 · 긍정 순간 10→3.
 *  원래 값(설치 3일 + 실행 5회 + **할 일 완료 10회**)은 월간 활성 기기가 한 자릿수인 지금
 *  사실상 아무도 못 채웠다. 스토어 실측: iOS 평점 2개 · Play 0개(전체 기간).
 *  긍정 순간에 "일정 등록 성공"도 더했다(eventService.createEvent) — 이 앱의 핵심 행동이다.
 *  🔑 경과일(3일)·재요청 간격·평생 한도는 그대로 둔다. "앱을 충분히 겪은 뒤 묻는다"는 Google 권고와
 *     OS 할당량 보호는 완화 대상이 아니다.
 */
export const REVIEW_POLICY = {
  ...
  minOpenCount: 3,
  /** 최소 긍정 순간 누적 횟수(할 일 완료 · 일정 등록 성공). */
  minPositiveCount: 3,
  /** 직전 요청 이후 최소 경과일. */
