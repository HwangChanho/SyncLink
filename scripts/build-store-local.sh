...
# 반영돼 있는지 확인한다.
#
# ⚠️ 적용 범위를 정확히 알고 쓸 것 (2026-08-10 오진 정정):
#   이 스크립트가 쓰는 `eas build --local` 은 프로젝트를 임시 디렉토리로 복사한 뒤
#   **자체 [PREBUILD] 단계에서 android/·ios/ 를 app.json 으로부터 새로 생성한다.**
#   따라서 로컬 android/ 가 아무리 낡아도 **스토어 산출물에는 영향이 없다.**
#
#   드리프트가 실제로 문제되는 경로는 로컬 디렉토리를 그대로 쓰는 쪽이다:
#     - fastlane beta / xcodebuild 직접  (08-09 iOS 아카이브 실패의 진짜 원인.
#       ios/ 가 SDK54 이전이라 Podfile.properties.json 의 newArchEnabled=false 였다)
#     - expo run:android 등 로컬 dev 빌드
#
#   즉 이 점검은 "이 스크립트로 나갈 산출물"보다 **로컬 네이티브 상태의 건강도**를 본다.
#   불일치가 보이면 다른 경로(fastlane 등)로 빌드할 때 깨질 상태라는 뜻이다.
#
#   🔴 08-10 에 이걸 반대로 읽었다: 로컬 gradle.properties 가 35 인 걸 보고
#   "vc18 이 정책 미달로 게시됐다" 고 단정했으나, Play 번들 표 확인 결과 vc18 도 36 이었다.
#   로컬 생성물의 값을 출시된 아티팩트의 값으로 단정하지 말 것.
#
# 불일치는 경고가 아니라 중단으로 둔다 — 어느 경로로 빌드하든 먼저 정리하는 편이 싸다.
# ---------------------------------------------------------------------------
config_drift() {
  echo "── 설정 드리프트 점검 ──"
  local drift=0
