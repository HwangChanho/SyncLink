...
function buildPrompt(slots: TimeSlot[]): string {
  ...
  // v1.2.9 — LEAD: "정해진 게 아닌 진짜 인사이트가 있는 제안".
  // 시간대별로 의미 있는 활동 (저녁=식사/와인, 오전=브런치/산책, 주말=나들이)
  // 을 추천 + 왜 그 시간이 좋은지 짧은 근거 (요일 패턴) 포함.
  return (
    `당신은 사용자의 일정 패턴을 읽고 의미 있는 모임/데이트 시간을 1개 추천하는 큐레이터입니다.\n` +
    `다음 후보 시간 중 가장 자연스러운 1개를 골라 한 문장 (35자 이내) 으로 추천하세요.\n` +
    `- 시간대 의미 반영: 평일 저녁=가벼운 외식/와인, 주말 오전=브런치/산책, 주말 오후=나들이/카페.\n` +
    `- "왜 이 시간이 좋은지" 짧은 근거를 곁들이세요 (예: "주말 오후라 여유롭게").\n` +
    `- 형식적 인사 ("좋은 하루") 금지. 친근하지만 인사이트 있는 톤.\n\n` +
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (isCreditError(err)) {
      ...
      await alertCreditExhausted(alertClient, { fn: 'suggest-date' });
