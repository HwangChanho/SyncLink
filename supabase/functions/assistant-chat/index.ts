...
const buildSystemPrompt = (locale: string, nowIso: string): string => {
  ...
  if (lang === 'ko') {
    return [
      ...
      '  추출해 createEvent 도구로 순서대로 등록한다.',
      '- 각 일정의 날짜는 캡처에 보이는 월/일 + 현재 연도 기준. 종료 시간이 명시 안',
      '  되면 시작 시간 + 1시간으로.',
      '- 시간 없이 종일만 적힌 일정은 allDay=true 로 등록.',
      '- 너무 많아 부담스러우면 우선 상위 5개만 등록한 뒤 사용자에게 "더 가져올까요?"',
      '  로 확인. 중복 등록 방지 위해 같은 제목+시작시각 조합은 한 번만.',
      '- 등록 완료 후 "X개 일정을 가져왔어요. 캘린더에서 확인해 보세요." 같이 요약.',
    ].join('\n');
  }
  return [
